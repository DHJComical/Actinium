...
![光影效果对比占位图](docs/images/showcase-02.png)
...
构建产物可放入兼容的 Cleanroom Minecraft 实例中测试。

## 仓库结构

- `src/`：Actinium 与 Celeritas 集成相关的主要代码。
- `glsm/`：内嵌的 GLSM 侧集成代码。
- `GTNHLib/`：项目使用的 GTNHLib 相关代码。
- `docs/`：开发记录、兼容性缺口和后续文档。
- `buildSrc/`：Gradle 构建逻辑和依赖定义。
