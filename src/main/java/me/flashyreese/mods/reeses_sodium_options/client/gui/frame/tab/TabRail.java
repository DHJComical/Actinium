...
final class TabRail {
    ...
    private void activateHeader(TabGroup group, Consumer<Tab<?>> tabSelector, Runnable rebuildFrame) {
        switch (this.groupModel.collapseMode()) {
            case SELECTED_GROUP -> {
                ...
                tabSelector.accept(group.tabs().get(0));
            }
            case ALL_EXPANDED -> tabSelector.accept(group.tabs().get(0));
            case MANUAL -> {
    ...
    private int calculateWidth(LayoutBounds frameDim) {
        ...
        return clamp(preferredWidth, TabFrame.TAB_RAIL_MIN_WIDTH, maximumWidth);
    }
    ...
    private int getTextWidth(Tab<?> tab) {
        int tabTitleWidth = Minecraft.getMinecraft().fontRenderer.getStringWidth(tab.getTitle().getString());
        if (!this.groupModel.shouldShowHeaders()) {
            return tabTitleWidth;
        }

        int headerTextWidth = Minecraft.getMinecraft().fontRenderer.getStringWidth(tab.getModOptions().name());
        if (ReeseSodiumOptionsConfig.config().isTabHeaderVersionLabels()) {
            headerTextWidth = Math.max(headerTextWidth, Minecraft.getMinecraft().fontRenderer.getStringWidth(tab.getModOptions().version()));
        }
