...
final class TabSelectionState {
    ...
    TabSelectionState(List<Tab<?>> tabs, Holder<String> persistedTabKey, GuiScreen screen) {
        this.tabs = tabs;
    ...
    void ensureSelectedTab(Consumer<Tab<?>> selectedTabConsumer) {
        ...
        this.selectedTab = Optional.ofNullable(this.tabs.get(0));
    ...
    void activateSelectedTab() {
        this.selectedTab.ifPresent(value -> {
            if (value.getPage() instanceof ExternalPage externalPage) {
                externalPage.getScreenConsumer().accept(this.screen);
            } else {
    ...
    OptionRow findSelectedOptionRow(ResourceLocation optionId) {
        ...
        return this.selectedFrame.findFirstOptionRow(control -> {
            String id = control.getOption().rso$getId();
            return id != null && !id.isEmpty() && optionId.equals(new ResourceLocation(id));
        });
    }
