...
public final class VaoAppleLwjgl3Fallback implements VaoFunctions {
    private static final int GL_VERTEX_ARRAY_BINDING_APPLE = 0x85B5;
    ...
    private static final class Lwjgl3Handles {
        ...
        static {
            ...
            try {
                ...
                stackCallocInt = lookup.findVirtual(memoryStackClass, "callocInt", MethodType.methodType(IntBuffer.class, int.class));
                stackInts = lookup.findVirtual(memoryStackClass, "ints", MethodType.methodType(IntBuffer.class, int.class));
                stackClose = lookup.findVirtual(memoryStackClass, "close", MethodType.methodType(void.class));
                memAddress = lookup.findStatic(memoryUtilClass, "memAddress", MethodType.methodType(long.class, Buffer.class));
                jniCallPV = lookup.findStatic(jniClass, "callPV", MethodType.methodType(void.class, int.class, long.class, long.class));
                jniCallV = lookup.findStatic(jniClass, "callV", MethodType.methodType(void.class, int.class, long.class));
                jniCallZ = lookup.findStatic(jniClass, "callZ", MethodType.methodType(boolean.class, int.class, long.class));

                Class<?> glClass = Class.forName("org.lwjgl.opengl.GL");
                Method getFunctionProviderMethod = glClass.getMethod("getFunctionProvider");
                Object provider = getFunctionProviderMethod.invoke(null);
                if (provider != null) {
    ...
    public static VaoFunctions tryCreate() {
        ...
        try {
            ...
            IrisGlDebug.logDebugInfo("universal-vao mode=lwjgl3-apple-fallback");
    ...
    public int getCurrentBinding() {
        return GL11.glGetInteger(GL_VERTEX_ARRAY_BINDING_APPLE);
    }
