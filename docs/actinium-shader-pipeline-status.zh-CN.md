...
Actinium 目前已经可以以可用状态运行 MakeUp Ultra Fast 光影包。核心世界渲染、天空、水面、后处理、GUI 隔离、TAA 兼容和体积云贴图采样都已经经过多轮修复。最新反馈状态是：场景渲染正常，体积云不再向四个方向无限拉伸。
...
该提交之后仍有未提交修改，主要集中在后处理 sampler 兼容、post target 深度支持、`prepare/deferred` 处理，以及云相关行为。
...
- advanced shadows 和 lighting effects。
- 依赖更完整 terrain gbuffers 的光影包水面和反射行为。

当前保守方案避免了大量严重回归，但也意味着许多高阶效果仍然不会出现。
...
- 在确认最新云修复后提交当前工作区改动。
- 继续用 MakeUp 的高设置测试：体积云、水反、TAA、Bloom、DOF、volumetric light。
- 如果效果缺失但场景稳定，增加针对 post target flip 的定向 debug。
