...
当前工作基线已经包含 2026-04-26 之后这几轮工作：光影包配置按包 `.txt` 持久化、天气接入、雨滴飞溅接线、阴影/运行时控制、地形性能恢复、TAA 相机历史稳定化，以及恢复 MakeUp 雾辅助路径的 `prepare` 元数据解析。
...
- 将 shader pack 的 `weather` 和 `weatherParticles` 指令真正接入运行时，而不再把天气行为视为恒定的原版逻辑。
- 当光影包提供 weather program 时，让雨雪渲染走 shader world-stage 路径。
- 让雨滴飞溅粒子进入 shader-aware 渲染阶段，并为彻底禁用飞溅粒子的光影包增加专门的拦截控制。
...
- 恢复独立的 pre-scene `prepare` 和 `deferred` 执行路径，其中 `prepare` 在 terrain setup 前运行，`deferred` 在水面/半透明阶段前运行。
- 修复宏保护下 draw-buffer 指令的元数据求值。`defined MACRO` 和 `defined(MACRO)` 现在都会被正确识别，从而让 MakeUp 的 `prepare` 输出恢复为 `drawBuffers=[1, 7]`。
- 恢复 `prepare` 写出的 `gaux4` 雾辅助传播路径，使 MakeUp 的雾和氛围数据重新进入后续 world/post 阶段。
- 继续对 pre-scene target 的回写做保守控制，避免 `prepare` 的颜色结果用 sky/prepass 颜色把整个世界冲成奶白色。

### 性能和运行时控制

- 收紧 shadow pass 裁剪和光影包/运行时阴影覆盖项，改善 shadow distance、entity shadow distance、block-entity shadow 和 hardware filtering 等控制项的兼容性。
- 通过收敛不必要的 terrain override 工作、移除热点路径上昂贵的临时 debug 探针，恢复了相当一部分开光影后的地形性能。
- 继续保持 solid/cutout 路径的保守地形策略，同时保留当前 MakeUp 依赖的 translucent/water 光影集成。

### TAA

- 参考 Iris 26.1 改善了 MakeUp 的 TAA 行为。
- 稳定了 terrain、world 和 post programs 之间的上一帧相机历史上传，包括部分光影包需要的整数/小数拆分相机位置 uniform。
...
- 将 shader 选项覆盖持久化迁移到与光影包同级的 `<shaderpack>.txt` 文件，不再把所有覆盖值集中存放在共享 config 中。
...
- 跟踪渲染阶段：world、sky、clouds、weather、particles、shadow、post、final。
...
- 执行 pre-scene `prepare`/`deferred` programs，以及 scene composite/final programs。
- 在 shader pass 后恢复 OpenGL/framebuffer 状态，并在需要时把持久化的 pre-scene 输出同步到后续阶段。
...
同一套 target 模型也被复用到独立的 pre-scene `prepare`/`deferred` 捕获路径。
...
- 开始 world pass。
- 在 terrain setup 前运行 `prepare`。
- 在水面/半透明阶段前运行 `deferred`。
- 捕获 world state，并在第一人称手部渲染前执行 post。
- 在光影包提供相关 program 时，开始/结束 particle 和 weather shader stage。
...
- 拦截 `EntityRenderer.addRainParticles()`。
- 当当前光影包禁用飞溅粒子时，阻止原版雨滴飞溅粒子生成。
...
- 诸如 distance multipliers 和 hardware filtering 的运行时阴影调优参数
- `prepareBeforeShadow`
...
当前行为仍然是面向兼容性的 pre-scene 实现，而不是完整的 Iris 复刻：

- `ENABLE_PRE_SCENE_PIPELINES = true`，因此 `renderPreparePipeline()` 会在 terrain setup 前运行，`renderDeferredPipeline()` 会在 `renderWorldPass` 的水面/半透明部分之前运行。
- 每个阶段都会把当前 scene 和 depth 复制到专门的 pre-scene targets，应用 `prepare_pre` / `deferred_pre` flips，并且只在对应 stage program 存在时才执行。
- `prepare` 现在可以保留 `colortex1` 和 `gaux4` 两类输出。恢复后的 `gaux4` 路径会把光影包的雾辅助数据继续传给后续 world/post 阶段。
- pre-scene 输出仍然采用保守回写策略，避免用 sky/prepass 颜色替换真实场景，从而再次触发奶白世界回归。

原因：

- MakeUp 的 `prepare_fragment.glsl` 依赖宏保护下的元数据；解析器现在同时支持 `defined MACRO` 和 `defined(MACRO)`，因此该阶段会正确解析为 `drawBuffers=[1, 7]`，而不是静默丢失 `gaux4`。
- 让 `prepare` 和 `deferred` 回到主场景之前执行，可以恢复雾和氛围行为，同时不再重新引入旧的全屏泛白问题。
- 相比 Iris，当前的 buffer ownership 仍然偏保守，因为 Actinium 只会合并那些应当继续存活到后续 post pass 的 targets。

### 光影包选项覆盖

- shader 选项覆盖现在存储为 `shaderpacks/<packName>.txt`。
- 覆盖值会按光影包单独加载；当覆盖项为空时，对应文件会被删除，因此不同光影包不再共用一个全局覆盖桶。
...
- `prepare_fragment.glsl` 使用了宏保护下的 draw-buffer 元数据，在当前工作路径中会解析为 `drawBuffers=[1, 7]`，对应 `colortex1` 和 `gaux4`。
...
当前 `prepare/deferred` 路径比之前更接近 Iris，但仍然是兼容折中：

- Iris 在 shadows 后运行 `prepare`，在 translucents 前运行 `deferred`。
- Actinium 现在会在 terrain 前和水面/半透明前运行独立的 pre-scene 阶段，这对 MakeUp 来说更接近预期，但仍不是逐阶段一比一复刻。
- scene/depth capture 和输出合并仍然是选择性的，因此依赖精确阶段间 buffer ownership 的光影包仍可能暴露边界问题。

后续应在 framebuffer state 和 terrain output 足够稳定后，继续让阶段顺序和 buffer ownership 更接近 Iris。
...
- 持续同步文档中的已验证运行状态，尤其是 pre-scene programs、雾辅助链路和天气行为。
- 继续用 MakeUp 的高设置测试：体积云、水反、阴影、TAA、Bloom、DOF、volumetric light、雨雪、飞溅粒子和雾过渡。
- 仅在出现新的回归时增加定向 debug；这轮阴影接通过程中加入的临时 debug 路径应继续服从运行稳定性。

### 中期

- 继续让 `prepare` 和 `deferred` 的顺序与 buffer ownership 更接近 Iris，尤其是 shadow-to-prepare 时机以及 deferred 的 scene/depth 输入。
- 在不重新打开不稳定的大范围 terrain redirect 的前提下，继续分析并缩小开光影后的 terrain/world-stage 性能差距。
