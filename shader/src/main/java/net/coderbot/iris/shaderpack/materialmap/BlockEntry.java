...
public class BlockEntry {
	...
	public static BlockEntry parse(@NotNull String entry) {
		...
		if (splitStates.length == 2 && !startsWithNumeric(splitStates[1])) {
		...
		if (startsWithNumeric(splitStates[1])) {
	...
	private static boolean startsWithNumeric(String value) {
		return !value.isEmpty() && StringUtils.isNumeric(value.substring(0, 1));
	}
