...
public final class UniversiumShader extends ShaderProgram {
    ...
    public void use() {
        ...
        if (shouldRenderInventory) {
            ...
            if (mc.player == null) {
                ...
                float yaw = (float) ((mc.player.rotationYaw * 2 * Math.PI) / 360.0);
                GL20.glUniform1f(location_yaw, yaw);

                float pitch = -(float) ((mc.player.rotationPitch * 2 * Math.PI) / 360.0);
    ...
    public UniversiumShader setLightLevel(float r, float g, float b) {
        lightLevel.set(MathHelper.clamp(r, 0.0f, 1.0f), MathHelper.clamp(g, 0.0f, 1.0f), MathHelper.clamp(b, 0.0f, 1.0f));
        return this;
    }
